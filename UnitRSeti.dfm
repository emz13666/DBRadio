object frmRadioseti: TfrmRadioseti
  Left = 146
  Top = 92
  Width = 843
  Height = 480
  Caption = #1056#1072#1076#1080#1086#1089#1077#1090#1080
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  KeyPreview = True
  OldCreateOrder = False
  Position = poMainFormCenter
  Visible = True
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  PixelsPerInch = 96
  TextHeight = 13
  object ToolBar1: TToolBar
    Left = 0
    Top = 0
    Width = 835
    Height = 29
    ButtonHeight = 24
    ButtonWidth = 25
    Caption = 'ToolBar1'
    TabOrder = 1
    object BtnDel: TButton
      Left = 0
      Top = 2
      Width = 75
      Height = 24
      Caption = #1059#1076#1072#1083#1080#1090#1100
      TabOrder = 1
      OnClick = BtnDelClick
    end
    object BtnChTP: TButton
      Left = 75
      Top = 2
      Width = 82
      Height = 24
      Caption = #1063#1058#1055
      TabOrder = 0
      OnClick = BtnChTPClick
    end
    object btADD: TButton
      Left = 157
      Top = 2
      Width = 75
      Height = 24
      Caption = #1044#1086#1073#1072#1074#1080#1090#1100'...'
      TabOrder = 2
      OnClick = btADDClick
    end
    object btEdit: TButton
      Left = 232
      Top = 2
      Width = 102
      Height = 24
      Caption = #1056#1077#1076#1072#1082#1090#1080#1088#1086#1074#1072#1090#1100'...'
      TabOrder = 3
      OnClick = btEditClick
    end
    object btnPoisk: TBitBtn
      Left = 334
      Top = 2
      Width = 75
      Height = 24
      Caption = #1053#1072#1081#1090#1080
      TabOrder = 4
      OnClick = btnPoiskClick
    end
    object btnFiltr: TBitBtn
      Left = 409
      Top = 2
      Width = 75
      Height = 24
      Caption = #1060#1080#1083#1100#1090#1088
      TabOrder = 5
      OnClick = btnFiltrClick
    end
    object btUpload: TBitBtn
      Left = 484
      Top = 2
      Width = 75
      Height = 24
      Caption = #1054#1073#1085#1086#1074#1080#1090#1100
      TabOrder = 6
      OnClick = btUploadClick
    end
    object ToolButton1: TToolButton
      Left = 559
      Top = 2
      Width = 8
      Caption = 'ToolButton1'
      Style = tbsSeparator
    end
    object BitBtn1: TBitBtn
      Left = 567
      Top = 2
      Width = 26
      Height = 24
      Hint = #1069#1082#1089#1087#1086#1088#1090' '#1074' Excel'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 7
      TabStop = False
      OnClick = PrintClick
      Glyph.Data = {
        
        0000000000000000000000000000}
    end
  end
  object GridRSeti: TRxDBGrid
    Left = 0
    Top = 29
    Width = 835
    Height = 398
    Align = alClient
    DataSource = DM.DSSeti
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
    ParentFont = False
    PopupMenu = PopupMenu1
    TabOrder = 0
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = [fsBold]
    OnDrawColumnCell = GridRSetiDrawColumnCell
    OnDblClick = GridRSetiDblClick
    ShowGlyphs = False
    TitleButtons = True
    OnTitleBtnClick = GridRSetiTitleBtnClick
    Columns = <
      item
        Expanded = False
        FieldName = 'number'
        Title.Caption = #8470
        Width = 25
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PodrLook'
        Title.Caption = #1055#1086#1076#1088#1072#1079#1076#1077#1083#1077#1085#1080#1077
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'freq'
        Title.Caption = #1063#1072#1089#1090#1086#1090#1099
        Width = 175
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'num_razr'
        Title.Caption = #8470' '#1088#1072#1079#1088#1077#1096#1077#1085#1080#1103
        Width = 96
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'date_end_razr'
        Title.Caption = #1056#1072#1079#1088#1077#1096#1077#1085#1080#1077' '#1076#1077#1081#1089#1090#1074'. '#1076#1086
        Width = 150
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Status_calc'
        Title.Caption = #1057#1090#1072#1090#1091#1089
        Width = 47
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'prim'
        Title.Caption = #1055#1088#1080#1084#1077#1095#1072#1085#1080#1077
        Width = 250
        Visible = True
      end>
  end
  object StatusBar1: TStatusBar
    Left = 0
    Top = 427
    Width = 835
    Height = 19
    Panels = <
      item
        Width = 50
      end>
    SimplePanel = False
  end
  object FormStorage1: TFormStorage
    UseRegistry = True
    StoredProps.Strings = (
      'GridRSeti.Columns')
    StoredValues = <>
    Left = 216
    Top = 152
  end
  object PopupMenu1: TPopupMenu
    Left = 280
    Top = 152
    object N1: TMenuItem
      Caption = #1063#1058#1055' '#1088#1072#1076#1080#1086#1089#1077#1090#1080
      OnClick = BtnChTPClick
    end
    object N2: TMenuItem
      Caption = #1056#1069#1057#1099' '#1088#1072#1076#1080#1086#1089#1077#1090#1080
      OnClick = N2Click
    end
    object N3: TMenuItem
      Caption = #1040#1073#1086#1085#1077#1085#1090#1099' '#1088#1072#1076#1080#1086#1089#1077#1090#1080
      OnClick = N3Click
    end
    object N4: TMenuItem
      Caption = '-'
    end
    object N5: TMenuItem
      Caption = #1047#1072#1082#1088#1099#1090#1100
      OnClick = N5Click
    end
  end
  object RSeti_detail: TSQLClientDataSet
    CommandText = 'radioseti'
    Aggregates = <>
    IndexFieldNames = 'number'
    Options = [poAllowCommandText]
    ObjectView = True
    Params = <>
    OnCalcFields = RSeti_detailCalcFields
    CommandType = ctTable
    DBConnection = DM.DBConnection
    Left = 280
    Top = 192
    object RSeti_detailid_radioseti: TFMTBCDField
      FieldName = 'id_radioseti'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      Precision = 20
      Size = 0
    end
    object RSeti_detailid_podrazd: TFMTBCDField
      FieldName = 'id_podrazd'
      Precision = 20
      Size = 0
    end
    object RSeti_detailnumber: TIntegerField
      FieldName = 'number'
      Required = True
    end
    object RSeti_detailfreq: TStringField
      FieldName = 'freq'
      Size = 200
    end
    object RSeti_detailnum_razr: TStringField
      FieldName = 'num_razr'
      Size = 200
    end
    object RSeti_detaildate_end_razr: TDateField
      FieldName = 'date_end_razr'
    end
    object RSeti_detailprim: TStringField
      FieldName = 'prim'
      Size = 255
    end
    object RSeti_detailPodrLook: TStringField
      FieldKind = fkCalculated
      FieldName = 'PodrLook'
      Calculated = True
    end
  end
  object DS_detail: TDataSource
    DataSet = RSeti_detail
    Left = 248
    Top = 152
  end
  object RSeti_query: TSQLClientDataSet
    Aggregates = <>
    Options = [poAllowCommandText]
    ObjectView = True
    Params = <>
    DBConnection = DM.DBConnection
    Left = 248
    Top = 192
  end
end
